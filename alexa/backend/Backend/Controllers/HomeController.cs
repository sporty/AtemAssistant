...
namespace LiveStreamAssistance
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> YouTubePlayList([FromServices] IGoogleAuthProvider auth)
        {
            ...
            var liveStream = await youTubeStream.Create(
                "New Broadcast",
                "This is New Broadcast",
                DateTime.Now,
                "unlisted",
                "New Stream", "./image/thumbnail.png",
                "", "");
        ...
        public async Task<IActionResult> CalendarList([FromServices] IGoogleAuthProvider auth)
        {
            ...
            if (await auth.RequireScopesAsync(CalendarService.ScopeConstants.CalendarReadonly) is IActionResult
                authResult)
            {
        ...
        [Authorize]
        [HttpPost]
        public async Task<IActionResult> ForceTokenRefreshCheckPersisted([FromServices] IGoogleAuthProvider auth,
            [FromForm] string expectedAccessToken)
        {
